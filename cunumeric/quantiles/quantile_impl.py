...
def averaged_inverted_cdf(q, n):
    ...
    k = cu.floor(pos)
    eps = 0.5
    gamma = 0.5 if cu.abs(k - pos) < eps else 1.0
    j = floor_i(k)
    return (gamma, j)
...
#
def quantile_impl(arr, q_arr, axis, method, keepdims):
    ...
    #
    # arr.dtype dependency:
    #
    # TODO: fix the behavior below; it actually depends on interpolation method;
...
def quantile(a, q, axis=None, out=None, overwrite_input=False, method='linear', keepdims=False, *, interpolation=None):
    dict_methods = {'inverted_cdf':inverted_cdf,
                    'averaged_inverted_cdf':inverted_cdf, # this is probably a BUG in numpy, but they use the same method here as previous
    ...
    res = quantile_impl(arr, q_arr, real_axis, dict_methods[method], keepdims)
